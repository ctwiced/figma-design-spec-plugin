...
// --- Constants ---
const PADDING_COLOR = { r: 0.74, g: 0.31, b: 0.95 };
const GAP_COLOR = { r: 0.949, g: 0.6, b: 0.29 };
const TITLE_COLOR = { r: 0, g: 0, b: 0 };
const CIRCLE_TEXT_COLOR = { r: 1, g: 1, b: 1 };
const LEGEND_BLUE = { r: 0, g: 0.408, b: 0.98 }; // #0068FA
const SWATCH_STROKE_COLOR = { r: 0.2, g: 0.2, b: 0.2 };
...
function createStyledText(characters, isBold = false) {
...
async function createValueCircle(value, color, unit = '') {
...
function findNodesRecursive(nodes, predicate) {
    let collection = [];
...
function createSpecSheetFrame(title, content) {
...
function createHorizontalContainer(sheets) {
    if (sheets.length === 0)
        return null;
    const container = figma.createFrame();
...
function getVisibleNodeIds(rootNode, focusNodes) {
...
function applyFocusOpacity(rootClone, originalRoot, focusNodes) {
    const visibleNodeIds = getVisibleNodeIds(originalRoot, focusNodes);
    function recursiveDim(originalNode, clonedNode) {
        if (visibleNodeIds.has(originalNode.id)) {
            ...
            if ('opacity' in clonedNode) {
                clonedNode.opacity = 0.2;
            }
...
async function drawNumberedLegendAnnotations(nodes, rootBounds) {
    const annotations = [];
    ...
    const quadrants = { top: [], bottom: [], left: [], right: [] };
    for (const node of nodes) {
    ...
    let legendCounter = 1;
    const processQuadrant = async (nodes, side) => {
        if (nodes.length === 0)
            return;
        // --- START: MODIFIED CODE ---
...
// --- LAYOUT SPECS ---
async function createLayoutSpecInfoFrame(frame) {
    const container = figma.createFrame();
    ...
    container.counterAxisSizingMode = 'AUTO';
    container.fills = [];
    container.appendChild(createStyledText(`Frame: "${frame.name}"`));
    const spacer = figma.createRectangle();
    spacer.resize(1, 16);
    spacer.fills = [];
    container.appendChild(spacer);
    container.appendChild(createStyledText("Layout Specs:", true));
    const getVarName = async (obj, key) => {
        var _a;
        const boundVar = (_a = obj.boundVariables) === null || _a === void 0 ? void 0 : _a[key];
    ...
    if (frame.paddingTop > 0)
        specStr += `• Top Padding: ${frame.paddingTop}px${await getVarName(frame, 'paddingTop')}\n`;
    if (frame.paddingBottom > 0)
        specStr += `• Bottom Padding: ${frame.paddingBottom}px${await getVarName(frame, 'paddingBottom')}\n`;
    if (frame.paddingLeft > 0)
        specStr += `• Left Padding: ${frame.paddingLeft}px${await getVarName(frame, 'paddingLeft')}\n`;
    if (frame.paddingRight > 0)
        specStr += `• Right Padding: ${frame.paddingRight}px${await getVarName(frame, 'paddingRight')}\n`;
    ...
    if (specStr)
        container.appendChild(createStyledText(specStr.trim()));
    return container;
}
async function drawPaddingAnnotations(focusFrame, topLevelFrame) {
    const createdNodes = [];
    const EXTENSION = 24;
    const GAP_FROM_LINE = 4;
    const { absoluteTransform: focusAbsT, width: focusW, height: focusH } = focusFrame;
    const { absoluteBoundingBox: parentBounds } = topLevelFrame;
    if (!parentBounds)
        return [];
    const focusAbsX = focusAbsT[0][2];
    const focusAbsY = focusAbsT[1][2];
    const lineStartX = parentBounds.x - EXTENSION;
    const lineEndX = parentBounds.x + parentBounds.width + EXTENSION;
    const lineStartY = parentBounds.y - EXTENSION;
    const lineEndY = parentBounds.y + parentBounds.height + EXTENSION;
    const paddings = { Top: focusFrame.paddingTop, Bottom: focusFrame.paddingBottom, Left: focusFrame.paddingLeft, Right: focusFrame.paddingRight };
    for (const side in paddings) {
        const value = paddings[side];
        if (value <= 0)
            continue;
        const rect = figma.createRectangle();
        rect.fills = [{ type: 'SOLID', color: PADDING_COLOR, opacity: 0.3 }];
        if (side === 'Top') {
            rect.resize(focusW, value);
            rect.x = focusAbsX;
            rect.y = focusAbsY;
            createdNodes.push(createLine(lineStartX, focusAbsY, lineEndX, focusAbsY, PADDING_COLOR), createLine(lineStartX, focusAbsY + value, lineEndX, focusAbsY + value, PADDING_COLOR));
        }
        else if (side === 'Bottom') {
            rect.resize(focusW, value);
            rect.x = focusAbsX;
            rect.y = focusAbsY + focusH - value;
            createdNodes.push(createLine(lineStartX, rect.y, lineEndX, rect.y, PADDING_COLOR), createLine(lineStartX, rect.y + value, lineEndX, rect.y + value, PADDING_COLOR));
        }
        else if (side === 'Left') {
            rect.resize(value, focusH);
            rect.x = focusAbsX;
            rect.y = focusAbsY;
            createdNodes.push(createLine(focusAbsX, lineStartY, focusAbsX, lineEndY, PADDING_COLOR), createLine(focusAbsX + value, lineStartY, focusAbsX + value, lineEndY, PADDING_COLOR));
        }
        else if (side === 'Right') {
            rect.resize(value, focusH);
            rect.x = focusAbsX + focusW - value;
            rect.y = focusAbsY;
            createdNodes.push(createLine(rect.x, lineStartY, rect.x, lineEndY, PADDING_COLOR), createLine(rect.x + value, lineStartY, rect.x + value, lineEndY, PADDING_COLOR));
        }
        const iconGroup = await createValueCircle(value, PADDING_COLOR, 'px');
        if (side === 'Top' || side === 'Bottom') {
            iconGroup.x = lineEndX + GAP_FROM_LINE;
            iconGroup.y = rect.y + rect.height / 2 - iconGroup.height / 2;
        }
        else {
            iconGroup.x = rect.x + rect.width / 2 - iconGroup.width / 2;
            iconGroup.y = lineEndY + GAP_FROM_LINE;
        }
        createdNodes.push(iconGroup, rect);
    }
    return createdNodes;
}
async function drawItemSpacingAnnotations(focusFrame, topLevelFrame) {
    if (focusFrame.itemSpacing <= 0 || focusFrame.children.length < 2 || focusFrame.primaryAxisAlignItems === 'SPACE_BETWEEN')
        return [];
    const createdNodes = [];
    const EXTENSION = 24;
    const GAP_FROM_LINE = 4;
    const value = focusFrame.itemSpacing;
    const { absoluteBoundingBox: parentBounds } = topLevelFrame;
    if (!parentBounds)
        return [];
    const lineStartX = parentBounds.x - EXTENSION;
    ...
    for (let i = 0; i < focusFrame.children.length - 1; i++) {
        ...
        if ('absoluteTransform' in child && 'width' in child && 'height' in child) {
            const rect = figma.createRectangle();
            rect.fills = [{ type: 'SOLID', color: GAP_COLOR, opacity: 0.3 }];
            const childAbsT = child.absoluteTransform;
            const childAbsX = childAbsT[0][2];
            const childAbsY = childAbsT[1][2];
            if (focusFrame.layoutMode === 'VERTICAL') {
                const gapY = childAbsY + child.height;
                rect.resize(child.width, value);
                rect.x = childAbsX;
                rect.y = gapY;
                if (i === 0) {
                    createdNodes.push(createLine(lineStartX, gapY, lineEndX, gapY, GAP_COLOR), createLine(lineStartX, gapY + value, lineEndX, gapY + value, GAP_COLOR));
                    const iconGroup = await createValueCircle(value, GAP_COLOR, 'px');
                    iconGroup.x = lineEndX + GAP_FROM_LINE;
                    iconGroup.y = rect.y + rect.height / 2 - iconGroup.height / 2;
                    createdNodes.push(iconGroup);
                }
            }
            else {
                const gapX = childAbsX + child.width;
                rect.resize(value, child.height);
                rect.x = gapX;
                rect.y = childAbsY;
                if (i === 0) {
                    createdNodes.push(createLine(gapX, lineStartY, gapX, lineEndY, GAP_COLOR), createLine(gapX + value, lineStartY, gapX + value, lineEndY, GAP_COLOR));
                    const iconGroup = await createValueCircle(value, GAP_COLOR, 'px');
                    iconGroup.x = rect.x + rect.width / 2 - iconGroup.width / 2;
                    iconGroup.y = lineEndY + GAP_FROM_LINE;
                    createdNodes.push(iconGroup);
...
async function generateSingleLayoutSpec(focusFrame, topLevelFrame) {
...
async function findAllColorNodes(nodes, map) {
    var _a;
    for (const node of nodes) {
        if (!node.visible)
            continue;
        const props = ['fills', 'strokes'];
        for (const prop of props) {
            if (prop in node && Array.isArray(node[prop])) {
                for (const paint of node[prop]) {
                    if (paint.type === 'SOLID' && paint.visible && (paint.opacity === undefined || paint.opacity > 0)) {
                        ...
                        if ((_a = paint.boundVariables) === null || _a === void 0 ? void 0 : _a.color) {
                            ...
                            key = rgbToHex(paint.color.r, paint.color.g, paint.color.b);
                        }
                        if (key) {
                            ...
                            if (!map.has(key))
                                map.set(key, { value: paint.color, elements: new Map(), variable });
                            const colorEntry = map.get(key);
                            if (!colorEntry.elements.has(focusTarget.id))
                                colorEntry.elements.set(focusTarget.id, { node: focusTarget, props: new Set() });
        ...
        if ('children' in node)
            await findAllColorNodes(node.children, map);
    }
}
async function generateSingleColorSpec(colorData, key, topLevelFrame) {
...
async function findAllRadiusNodesGroupedByValue(nodes, map) {
    for (const node of nodes) {
        if (!node.visible)
            continue;
        let key = null;
        ...
        if ('boundVariables' in node && node.boundVariables) {
            ...
            for (const radiusKey of radiusKeys) {
                if (radiusKey in node.boundVariables) {
                    const boundVar = node.boundVariables[radiusKey];
                    if (boundVar && 'id' in boundVar) {
        ...
        if (!key)
            key = getRadiusString(node);
        if (key) {
            if (!map.has(key))
                map.set(key, { value: getRadiusString(node) || key, nodes: [], variable });
            map.get(key).nodes.push(node);
        }
        if ('children' in node)
            await findAllRadiusNodesGroupedByValue(node.children, map);
    }
}
async function createRadiusSpecInfoFrame(focusFrame, radiusData) {
...
async function drawRadiusAnnotations(focusFrame) {
    ...
    const focusAbsX = focusAbsT[0][2];
    const focusAbsY = focusAbsT[1][2];
    const RADIUS_ANNOTATION_COLOR = PADDING_COLOR;
    const radii = {
...
async function generateSingleRadiusSpec(radiusData, key, topLevelFrame) {
...
// --- TEXT TOKEN SPECS ---
async function findAllTextNodesGroupedByStyle(nodes, map) {
    for (const node of nodes) {
...
async function generateSingleTextSpec(textData, key, topLevelFrame) {
...
figma.ui.onmessage = async (msg) => {
    if (msg.type === 'generate-specs') {
        ...
        let layoutSheets = [], colorSheets = [], radiusSheets = [], textSheets = [];
        // --- END: CORRECTED LINE ---
        ...
        let totalWeight = 0;
        const isLayoutFrameToSpec = (node) => node.type === 'FRAME' && node.layoutMode !== 'NONE' && (node.paddingTop > 0 || node.paddingBottom > 0 || node.paddingLeft > 0 || node.paddingRight > 0 || (node.itemSpacing > 0 && node.children.length > 1 && node.primaryAxisAlignItems !== 'SPACE_BETWEEN'));
        const framesToSpec = layout ? findNodesRecursive([topLevelFrame], isLayoutFrameToSpec) : [];
        if (layout)
            totalWeight += framesToSpec.length * SPEC_WEIGHTS.layout;
        const allColorsMap = new Map();
        if (colors) {
            await findAllColorNodes([topLevelFrame], allColorsMap);
            totalWeight += allColorsMap.size * SPEC_WEIGHTS.colors;
        }
        const allRadiiMap = new Map();
        if (radius) {
            await findAllRadiusNodesGroupedByValue([topLevelFrame], allRadiiMap);
            totalWeight += allRadiiMap.size * SPEC_WEIGHTS.radius;
        }
        const allTextMap = new Map();
        if (text) {
            await findAllTextNodesGroupedByStyle([topLevelFrame], allTextMap);
            totalWeight += allTextMap.size * SPEC_WEIGHTS.text;
        }
        let completedWeight = 0;
        const updateProgress = async (weight) => {
        ...
        if (text && allTextMap.size > 0) {
            ...
            textSheets.push(...resolvedTextSheets.filter(sheet => sheet !== null));
        }
        ...
        if (layoutGroup || colorGroup || radiusGroup || textGroup) {
            ...
            masterContainer.layoutMode = 'VERTICAL';
            masterContainer.itemSpacing = 80;
            masterContainer.paddingTop = 50;
            masterContainer.paddingBottom = 50;
            masterContainer.paddingLeft = 50;
            masterContainer.paddingRight = 50;
            ...
            const createSection = (title, group) => {
                if (!group)
                    return;
